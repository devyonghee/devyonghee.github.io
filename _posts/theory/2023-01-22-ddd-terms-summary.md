...
- 특정 타입의 모든 객체를 개념적 집합 (객체 **컬렉션**처럼 동작)
...
{% include image.html alt="공유 커널" source_txt='도메인 주도 설계' path="images/theory/ddd-terms-summary/anticorruption-layer.png" %}

통신 및 전송 메커니즘, Facade, Adapter, 번역기 조합으로 설계

- Facade
  - 한쪽 모델에서 다른 모델로 번역하는 것을 담당
  - 하위 시스템에 대한 **클라이언트 접근을 단순**화하는 **인터페이스**
...
- Bounded Context 가 **다른 것과 관계를 맺지 않도록 선언**하여 범위 내에서 해결책을 찾음
...
- **핵심적인 부분**을 쉽게 파악할 수 있도록 Core Domain 을 더 잘 보이게끔 만드는 과정
- 디스틸레이션 문서 작성 기법
...
{% include image.html alt="대규모 구조의 패턴" source_txt='도메인 주도 설계' path="images/theory/ddd-terms-summary/large-scale-structure.png" %}
...
{% include image.html alt="지식 수준" source_txt='도메인 주도 설계' path="images/theory/ddd-terms-summary/knowledge-level.png" %}

- 모델의 **구조와 행위를 서술하고 제약**하는 데 쓸 수 있는 객체 집합
