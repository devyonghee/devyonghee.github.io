...
- 동시성 프로그래밍을 할때 안전성(safety)과 응답 가능(liveness) 상태로 유지해야 한다.  
- 스트림을 잘못 병렬활하면 성능이 나빠지고 결과 자체가 잘못되거나 예상치 못한 동작이 발생될 수 있다.  
- 스트림 병렬화는 오직 성능 최적화 수단이어야 한다.
- 운영 환경과 유사한 조건에서 성능이 좋아지고 정확도가 높을 때만 병렬화를 반영해야 한다. 


### 병렬화가 적합하지 않은 경우
  - 데이터 소스가 `Stream.iterate`거나 중간 연산으로 `limit`이 사용된 파이프라인은 성능 개선을 기대할 수 없음
  - 참조 지역성이 낮으면(실제 객체가 메모리에서 떨어져 있음) 스레드는 데이터가 주 메모리에서 캐시메모리로 전송되어 오기를 기다림
  - 종단 연산이 가변 축소 수행(ex. `Stream`의 `collect`)


### 병렬화 효과가 좋은 경우
- 스트림의 소스를 데이터를 원하는 크기로 정확하고 손쉽게 나눌 수 있는 경우  
  (ex. `ArrayList`, `HashMap`, `HashSet`, `ConcurrentHashMap`, 배열, int 범위, long 범위)
- 종단 연산이 축소(reduction) 인 경우 (ex. `reduce`, `max`, `min`, `count`, `anyMatch` ...)
