...
상위 클래스의 재정의하는 모든 메서드에 `Override` 애너테이션을 달아라

- 재정의해야 되는데 실수로 오버로딩을 하여 의도하지 않은 동작이 발생될 수 있음
- 컴파일시에 오류를 찾아낼 수 있음
- 인터페이스에 디폴트 메서드가 없다면 구현한 메서드에는 `Override` 생략하여 깔끔하게 유지해도 좋음

<br/>

## 아이템 41. 정의하려는 것이 타입이라면 마커인터페이스를 사용하라

자신을 구현하는 클래스가 특정 속성을 표시하는 인터페이스가 **마커 인터페이스**(ex. `Serializable`)이다.

### 마커인터페이스가 마커애너테이션보다 나은 점

- 인스턴스들을 구분하는 **타입**으로 사용가능
- 적용 대상을 정밀하게 지정할 수 있음
  - 마킹된 타입은 자동으로 하위 타입 보장

반대로 **마커 애너테이션**은 거대한 애너테이션 시스템(애너테이션을 적극적으로 활용하는 프레임워크)의 지원을 받음

### `Set` 도 일종의 마커 인터페이스

- `Collection`이 정의한 메서드 외에 추가된 메서드가 없음
- 특정 인터페이스 하위 타입에만 적용 가능
- 다른 특정 메서드가 처리할 수 있다는 사실을 명시하는 용도

마킹이 된 객체를 매개변수로 받는 메서드를 작성해야 된다면 **마커 인터페이스**  
위와 같은 메서드를 작성할 일이 없거나 프레임워크에서 사용하려는 마커라면 **마커 애너테이션**  
