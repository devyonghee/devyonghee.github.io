...
`setter` 와 `getter` 가 가지고 있는 데이터는 **단순 자료 구조(data structure)**이다. 
...
- `setter` 와 `getter` 는 캡슐화 원칙을 손쉽게 위반한다.
- 메서드라 부가적인 로직을 추가할 수 있지만, 데이터에 **직접 접근**할 수 있는 **진입점**과 동일하다.

### 3.5.3 접두사에 관한 모든 것

`setter` 와 `getter` 은 끔찍한 안티패턴이다. 절대 이렇게 짓지 말자

- `setter` 와 `getter` 접두사가 `get` 과 `set` 이라는 사실이 중요하다.
- `get` 접두사는 객체를 데이터의 저장소로 취급하는 것이다.
- `getDollars()`는 데이터를 노출하지만, `dollars()`는 노출하지 않는다.  

### Review

이번 섹션은 항상 의문을 가지고 있던 내용이었기 때문에 기분좋게 읽었다.
처음 개발을 공부할 때 캡슐화 개념과 함께 `setter` 와 `getter` 선언 하는 것을 배웠던 것 같다.
데이터를 숨기라면서 왜 `setter` 와 `getter`로 노출하지 라고 항상 이상하다고 여겼다.
